...
### Qt 原生版正式接管
- 完成 Qt 6 / QML / C++ 功能迁移，删除 Electron、React、Vite 与旧前端运行时，仓库、构建和发布链路统一为 Qt 版。
- 完整移除酷狗概念版的模式、登录、API 与界面入口，仅保留网易云音乐和 QQ 音乐。
- 登录采用轻量 WebView2 窗口自动捕获 Cookie，账号凭据由 Windows DPAPI 加密保存。

### 双平台、VIP 与播放稳定性
- 为网易云与 QQ 独立缓存首页、歌单和账号状态，平台切换即时回显，后台静默刷新。
- 账号管理显示双平台 VIP 类型、等级与到期时间；修复会员已登录仍获取 30 秒试听链接的问题。
- 增加已鉴权音源重试、试听链接识别、下一曲预取和不可播歌曲自动跳过，减少切歌停顿与播放器延迟。
- 修复歌单内点击歌曲后页面回到顶部，并保留操作前的滚动位置。

### 歌词与全屏视觉
- 完善网易云 YRC 与 QQ QRC/LRC 解析，恢复 QQ 歌词翻译，修复偶发只加载一行、长句异常换行和逐字高光丢失。
- 重构全屏歌词行切换，加入非线性海浪位移、错峰景深、逐字光晕和稳定行距，同时限制屏外动画以降低换行负载。
- 纯歌词/双栏布局改为立即切换，双栏歌词固定左边缘缩放，消除闪现、行距跳变与斜向阶梯。
- 完善歌曲列表、播放页和叠放封面的高 DPI 缩放、圆角裁剪与抗锯齿。

### 性能、缓存与发布
- 优化流体背景、歌词效果、封面/头像缓存与网络请求，增加缓存容量/时间清理和一键性能设置。
- 发布包改用最小 Node.js 双平台侧车依赖，部署 Qt、WebView2 和 app-local MSVC 运行库，提供可移植 ZIP 与 NSIS x64 安装版。


## [v1.0.8-Beta] - 2026-08-15

> ⚠️ **【Beta 测试版本特别说明】**：目前版本为 Beta 测试版，保留网易云音乐与 QQ 音乐双平台。
...
- 前奏三点倒计时（主流音乐 App 风格）覆盖网易云与 QQ；消失时机锚定真实开唱时间（不再受歌词行提前激活影响）。
...
- 全屏歌词前奏阶段新增三个圆点依次点亮倒计时（主流音乐 App 风格），第三个圆点亮起即开唱，不遮挡第一句歌词。
