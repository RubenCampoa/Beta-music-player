...
> ⚠️ **【Beta 测试版本特别说明】**：目前版本为 Beta 测试版，仅有网易云音乐完成正常适配；QQ 平台预计于 v1.0.9 完成深度适配；酷狗概念版适配进度待定。

### 酷狗概念版平台
- 新增酷狗概念版第三平台：支持关键词搜索、热门歌单、歌单详情、扫码/微信登录、会员音源解析与账号歌单同步。
- 新增酷狗 KRC 逐字歌词解析，接入全屏逐字点亮、歌词切换动效与前奏三点倒计时。
- 酷狗 API 作为本地服务随应用启动，固定监听 `127.0.0.1:3400`；搜索在设备验证不可用时自动切换官方移动接口兜底。
...
- 前奏三点倒计时（主流音乐 App 风格）覆盖网易云、QQ 与酷狗；消失时机锚定真实开唱时间（不再受歌词行提前激活影响）。
