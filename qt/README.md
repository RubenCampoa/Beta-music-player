...
项目仅保留 Qt 版本。界面使用 QML 与 Qt Quick Controls 自定义组件构建，
桥接层由原生 **C++ / Qt 6** 实现。
...
Multimedia、WebView2、网易云/QQ 推荐、搜索、歌单、账号、扫码与网页登录、VIP
...
首次构建前在本目录执行 `npm install` 安装最小侧车依赖。安装编译器后，先双击
`build_cpp.bat` 构建并部署 Qt 运行库，再双击
`run_cpp.bat` 启动。构建脚本会自动查找 Visual Studio，并在缺少 C++ 工具链时
显示明确错误，不会闪退。

执行 `package_qt.bat` 可复现生成 `dist/Beta Music Player 1.0.9 Portable.zip`
与 `dist/Beta Music Player Setup 1.0.9.exe`。发布物内含 Qt、WebView2、
Multimedia、Node.js、双平台生产依赖和 app-local MSVC 运行库，不依赖系统 Qt、
...
- 网易云、QQ 音乐真实推荐、搜索、歌单详情与即时平台切换
- 播放队列、收藏（含网易云账号同步）、音量、进度、随机与单曲/列表循环独立开关
- 双平台扫码、手动 Cookie 与 WebView2 独立网页登录 Cookie 捕获
- 登录后账户头像、昵称，以及网易云/QQ 个人歌单同步
- 双栏/纯歌词全屏界面，YRC/QRC 逐字歌词与歌词快慢偏移微调
- 桌面歌词悬浮窗（双行、锁定鼠标穿透、播放控制）
- 本地音乐文件/文件夹/拖放导入、去重、删除、ID3/封面/时长与同名 LRC 读取
- GPU ShaderEffect 流体背景和封面主色提取
- 设置项、音质、音量、播放模式与收藏的本地持久化

Qt 数据默认保存在 `%LOCALAPPDATA%\BetaMusicPlayerQt\storage.json`；升级会保留
现有账号、收藏、本地库和设置。侧车仅监听回环地址并使用每次启动的随机令牌认证。
